...
LOCAL_TIMELAPSE_DIR="./public/timelapse"
...
echo -e "${BLUE}Starting timelapse sync from printer...${NC}"

# Create local timelapse directory if it doesn't exist
mkdir -p "$LOCAL_TIMELAPSE_DIR"
...
download_file() {
    local filename="$1"
    local local_path="$LOCAL_TIMELAPSE_DIR/$filename"
    ...
    local remote_url="$PRINTER_API_URL/server/files/timelapse/$encoded_filename"
...
# Fetch list of timelapse files from printer
log "${BLUE}Fetching timelapse file list from printer...${NC}"

file_list=$(curl -s "$PRINTER_API_URL/server/files/list?root=timelapse")
...
if [ -z "$filenames" ]; then
    log "${YELLOW}No timelapse files found on printer${NC}"
    exit 0
fi

# Count total files
total_files=$(echo "$filenames" | wc -l | tr -d ' ')
log "${BLUE}Found $total_files timelapse files${NC}"
...
if [ "$1" = "--clean" ]; then
    ...
    if [ -d "$LOCAL_TIMELAPSE_DIR" ]; then
        for local_file in "$LOCAL_TIMELAPSE_DIR"/*; do
...
echo -e "${GREEN}Timelapse sync completed successfully!${NC}"